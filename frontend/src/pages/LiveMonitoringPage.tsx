...
export default function LiveMonitoringPage() {
  ...
  return <div className="stack-lg wide-page live-monitoring-page">
    ...
    <div className="monitor-summary"><span><i className="status-dot online"/> {filtered.filter((w) => w.online).length} online</span><span>{filtered.length} pekerja ditampilkan</span><span>Urutan: Online, Alfabetis</span></div>
    {filtered.length === 0 ? <EmptyState label="Tidak ada pekerja yang cocok dengan filter."/> : view === 'grid' ? <div className="worker-card-grid">{filtered.map((worker) => <WorkerCard key={worker.id} worker={worker} onMarkSafe={worker.risk_level === 'CRITICAL' ? () => markSafe.mutate(worker.id) : undefined}/>)}</div> : <div className="table-wrap"><table><thead><tr><th>Worker</th><th>Risk</th><th>Bahaya terdekat</th><th>RSSI</th><th>Proximity</th><th>Exposure</th><th>Environment</th><th>Device</th></tr></thead><tbody>{filtered.map((worker) => <tr key={worker.id} className={!worker.online ? 'offline-row' : ''} style={!worker.online ? { opacity: 0.5 } : {}}><td><a href={`/workers/${worker.id}`}><strong>{worker.name}</strong><small>{worker.id} · {worker.area}</small></a></td><td>{worker.online ? <><b className={`score score-${worker.risk_level.toLowerCase()}`}>{worker.risk_score}</b> <RiskBadge level={worker.risk_level}/></> : '-'}</td><td>{worker.online ? <><strong>{worker.hazard_name}</strong><small>{worker.hazard_status}</small></> : '-'}</td><td className="mono">{worker.online ? `${worker.rssi} dBm` : '-'}</td><td>{worker.online ? <RiskBadge level={worker.proximity}/> : '-'}</td><td>{worker.online ? `${worker.exposure_seconds} dtk` : '-'}</td><td>{worker.online ? `${worker.temperature}°C · ${worker.humidity}%` : '-'}</td><td><span className={`online-label ${!worker.online ? 'offline' : ''}`}><i/> {worker.online ? 'Online' : 'Offline'}</span></td></tr>)}</tbody></table></div>}
  </div>
}
